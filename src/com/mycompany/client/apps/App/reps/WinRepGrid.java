 ...
 */
public class WinRepGrid extends AReportCreator
{
...
//    private String dataURL = "transport/dataPlacesCons";
    ...
    public WinRepGrid(String headerURL, String dataURL, IGridMetaProvider gridMetaProvider)
    {
    ...
    public WinRepGrid(IGridMetaProvider gridMetaProvider)
    {
        this.gridMetaProvider = gridMetaProvider;
    }
...
//    }
    ...
    public void setHeaders(final Window portlet, final ListGridWithDesc grid) throws CacheException
    {
        ...
        dataId = dataId.replace("/", "$");
        final Criteria criteria = new Criteria(TablesTypes.TTYPE, TablesTypes.WINREP);
        DataSource.get(dataId).fetchData
        (
            criteria,
            new DSCallback()
                ...
                public void execute(DSResponse dsResponse, Object data, DSRequest dsRequest)
                {
                       ...
                       if (descData!=null && descData.length==1)
                       {
                           try {
                               ...
                               initData(grid,dataCallBack, TablesTypes.WINREP);
                           }
                           finally
                           {
                               completely=true;
...

                        {
                            ...
                            br:
                            {
                                ...
                                for (String letter : letter2Name.keySet())
                                {
                                    ...
                                    {
                                        {
                                            jsf.setExpressionValue("return srv.dHh("+name2l.get(ndName)+","+name2l.get(kdName)+")+Math.floor((srv.dMin("+name2l.get(ndName)+","+name2l.get(kdName)+")%60)/6)/10;");
                                        ...
                                        {
                                            jsf1.setExpressionValue("return srv.dHh(" + name2l.get(dt_ndName) + "," + name2l.get(dt_kdName) + ")+Math.floor((srv.dMin(" + name2l.get(dt_ndName) + "," + name2l.get(dt_kdName) + ")%60)/6)/10;");
...
//        owner.setTitle(title);
    ...
    public String getTitle(String strDate) {
        return "Отчет о выполнении окон на "+strDate;
    }
}
