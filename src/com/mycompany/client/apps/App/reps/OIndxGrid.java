 ...
 */
public class OIndxGrid extends AReportCreator
{
...
//    private String dataURL = "transport/dataCons";
    ...
    public OIndxGrid(String headerURL, String dataURL, IGridMetaProvider gridMetaProvider)
    {
    ...
    public OIndxGrid(IGridMetaProvider gridMetaProvider)
    {
        this.gridMetaProvider = gridMetaProvider;
    }
...
//                            else
                ...
                if (ix!=2 && ix!=3 && ix!=4)  //TODO Передавать индексы в аттрибутах
...
//    }
    ...
    public void setHeaders(final Window portlet,final ListGridWithDesc grid) throws CacheException
    {
        ...
        dataId = dataId.replace("/", "$");
        final Criteria criteria = new Criteria(TablesTypes.TTYPE, TablesTypes.OINDX);
        DataSource.get(dataId).fetchData
        (
            criteria,
            new DSCallback()
            ...
            //                                    получение данных из запросы
                       ...
                       if (descData!=null && descData.length==1)
                       {
                           try {
                               ...
                               initData(grid,dataCallBack, TablesTypes.OINDX);
                           }
                           finally
                           {
                               completely=true;
    ...
    {
        return "Оперативные показатели ДИ на "+strDate;
    }
}
