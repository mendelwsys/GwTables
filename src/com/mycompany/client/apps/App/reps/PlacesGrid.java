 ...
 */
public class PlacesGrid extends AReportCreator
{
...
//    private String headerURL="PlacesConsHeader.jsp";

    private IGridMetaProvider gridMetaProvider;

    public PlacesGrid(String headerURL, String dataURL, IGridMetaProvider gridMetaProvider)
    {
    ...
    public PlacesGrid(IGridMetaProvider gridMetaProvider)
    {
        this.gridMetaProvider = gridMetaProvider;
    }
    ...
    public void allocateHeaders(final ListGridWithDesc grid, IAnalisysDesc desc)
    {
        ...
        imgField.setCanEdit(false);
                ...
                //if (ListGridFieldType.IMAGE.getValue().equals(nnode.getType()))
                if ("ICONS".equals(nnode.getColId()))
                {
                    pdID.setType(ListGridFieldType.IMAGE);
                    pdID.setCanEdit(false);
                }
                else
                {
...
//    }
    ...
    public void setHeaders(final Window portlet, final ListGridWithDesc grid) throws CacheException
    {
        ...
        dataId = dataId.replace("/", "$");
        final Criteria criteria = new Criteria(TablesTypes.TTYPE, TablesTypes.STATEPLACES);
        DataSource.get(dataId).fetchData
        (
            criteria,
            new DSCallback()
                ...
                public void execute(DSResponse dsResponse, Object data, DSRequest dsRequest)
                {
                       ...
                       if (descData!=null && descData.length==1)
                       {
                           try {
                               ...
                               initData(grid,dataCallBack, TablesTypes.STATEPLACES);
                           }
                           finally
                           {
                               completely=true;
...
//        owner.setTitle(title);
    ...
    public String getTitle(String strDate) {
        return "Состояние инфраструктуры. Участки. на "+strDate;
    }
}
