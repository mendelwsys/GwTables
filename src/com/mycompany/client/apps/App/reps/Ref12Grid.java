 ...
 */
public class Ref12Grid extends AReportCreator
{

    public static final String INTERVAL_FIELD_NAME = "zIntervalVal_";
    Map<Integer,Record> dorCode2SumTotal = new HashMap<Integer,Record>();
    Record summTotal;


    //private String headerURL="Ref12ConsHeader.jsp";
...
//    private String dataURL = "transport/dataPlacesCons";
    ...
    public Ref12Grid(String headerURL, String dataURL, IGridMetaProvider gridMetaProvider)
    {
    ...
    public Ref12Grid(IGridMetaProvider gridMetaProvider)
    {
        this.gridMetaProvider = gridMetaProvider;
    }
...
//    }
    ...
    public void setHeaders(final Window portlet, final ListGridWithDesc grid) throws CacheException
    {
        ...
        dataId = dataId.replace("/", "$");
        final Criteria criteria = new Criteria(TablesTypes.TTYPE, TablesTypes.STATEREF12);
        DataSource.get(dataId).fetchData
        (
            criteria,
            new DSCallback()
                ...
                public void execute(DSResponse dsResponse, Object data, DSRequest dsRequest)
                {
                       ...
                       if (descData!=null && descData.length==1)
                       {
                           try {
                               ...
                               initData(grid,dataCallBack, TablesTypes.STATEREF12);
                           }
                           finally
                           {
                               completely=true;
...
//        new DrillRepOperation(-100, -100,"Вверх", IOperation.TypeOperation.addEventPortlet,false)
//        {
//            protected ArrowClickHandler createArrowClickHandler(final Canvas repGrid, final Window target)
//            {
...
//                            ((ListGridWithDesc)(items[0])).delJSFormula(INTERVAL_FIELD_NAME);
//                        super.onClick(event);
//                    }
//
//                };
        ...
        new PostponeOperationProvider(new PostponeOperationProvider.IPostponeOperation(){
            ...
            public boolean operate()
            {
                ...
                if (items!=null && items.length>0)
                {
                    ...
                    {
                        {
                            JSFormula jsf = tabGrid.createEmptyJSFormula(INTERVAL_FIELD_NAME);
                            ...
                            br:
                            {
                                ...
                                {
                                    if (letter2Name.get(letter).equals("ND"))
                                    {
                                        jsf.setExpressionValue("return srv.dcHh("+letter+")+Math.floor((srv.dcMin("+letter+")%60)/6)/10;");
                                        jsf.setFieldType(ListGridFieldType.FLOAT);
                                        jsf.setReCalcFormula(true);
                                        jsf.setPeriod(60000);
                                        jsf.setTitle("Истекло (час)");
...
//        owner.setTitle(title);
    ...
    public String getTitle(String strDate) {
        return "ОТС с начала суток (1 и 2 кат.) на "+strDate;
    }
}
