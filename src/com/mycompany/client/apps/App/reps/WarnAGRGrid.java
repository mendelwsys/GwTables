 ...
 */
public class WarnAGRGrid extends AReportCreator
{
...
//    private String dataURL = "transport/dataCons";
    ...
    public WarnAGRGrid(String headerURL, String dataURL, IGridMetaProvider gridMetaProvider)
    {
    ...
    public WarnAGRGrid(IGridMetaProvider gridMetaProvider)
    {
        this.gridMetaProvider = gridMetaProvider;
    }
                ...
                else
                {
                    ...
                    if (!tid.equals(tColId))
                    {
                        ...
                        pdID.setSortNormalizer(new SortNormalizer()
                        {
                            ...
                            {
                                return record.getAttributeAsInt(TablesTypes.DOR_CODE);
                            }
                        });
...
//    }
    ...
    public void setHeaders(final Window portlet,final ListGridWithDesc grid) throws CacheException
    {
        ...
        dataId = dataId.replace("/", "$");
        final Criteria criteria = new Criteria(TablesTypes.TTYPE, TablesTypes.STATEWARNAGR);
        DataSource.get(dataId).fetchData
        (
            criteria,
            new DSCallback()
            ...
            //                                    получение данных из запросы
                       ...
                       if (descData!=null && descData.length==1)
                       {
                           try {
                               ...
                               initData(grid,dataCallBack, TablesTypes.STATEWARNAGR);
                           }
                           finally
                           {
                               completely=true;
