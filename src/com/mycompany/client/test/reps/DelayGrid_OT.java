...
public class DelayGrid_OT
{
    ...
    public DelayGrid_OT(String headerURL, String dataURL, IGridMetaProvider gridMetaProvider)
    {
    ...
    public DelayGrid_OT(IGridMetaProvider gridMetaProvider)
    {
        this.gridMetaProvider = gridMetaProvider;
    }
                ...
                else
                {
                    ...
                    if (!tid.equals(tColId))
                    {
                        ...
                        if (!colDef.isHide())
                        {
                            ...
                            grpSpan.setHeight(1);//TODO Если ставить ноль делит видимую область попалам
//                            pdID.setTitle(String.valueOf(keyCols[0].getColN()));

//                            pdID.setShowTitle(false);
...
//TODO Тестовая функция удаления спанов из области видимостию.
            HeaderSpan[] viewValSpans = rootSpan.getSpans();
            final HeaderSpan[] spans = viewValSpans[0].getSpans();
            for (int i = 0,j=0,spansLength = spans.length; i < spansLength; i++,j++)
            {
                HeaderSpan span = spans[i];
                for (;j<ll.size();j++)
                    try
                    {
                        Integer.parseInt(ll.get(j).getName());
                        ll.get(j).setTitle(span.getTitle());
                        break;
                    } catch (NumberFormatException e) {
                        e.printStackTrace();
                    }
                span.setHeight(1);//TODO Если ставить ноль делит видимую область попалам
            }
