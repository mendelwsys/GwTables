package com.mwlib.tablo.derby.winrep;
 ...
 */
public class Consolidator4WinRepDataLoader implements IDataLoader
{
    ...
    public Consolidator4WinRepDataLoader(String[] loaderTypes, String dsName, boolean test)
    {
        this.test=test;
...
//TODO !!!!проверить политику заполнения кеша данных после получения консолидационных таблиц!!!!
            ...
            String time=sdf2.format(dt);
...

                {
                    ...
                    addInParams.put("EVENT_TYPE",addInParams.get("EVENT_TYPE")+"P");
                ...
                {
                    Map<String,Object> addInParams= new HashMap<String,Object>();
                    addInParams.put("EVENT_TYPE",tblName+"#WT");
                    String addStringFilter="STATUS_FACT=0  and STATUS_PL<>1\n" +
                                "and tbl.KD>=TIMESTAMP('"+stringDate+"','00:00')  \n" +
                                "and tbl.ND>=TIMESTAMP('"+stringDate+"','"+time+"')  \n" +
                                "and \n" +
                                "(\n" +
                                "\t(tbl.DT_KD>TIMESTAMP('1900-01-01','00:00') and {fn timestampdiff(SQL_TSI_MINUTE,tbl.DT_ND,tbl.DT_KD)}>=60)\n" +
                                "\tor\n" +
                ...
                {//в работе
                    Map<String,Object> addInParams= new HashMap<String,Object>();
                    addInParams.put("EVENT_TYPE",tblName+"#WK");
                    String addStringFilter="STATUS_FACT=1  and STATUS_PL<>1\n" +
                            "and tbl.KD>=TIMESTAMP('"+stringDate+"','00:00')  \n" +
                            "and \n" +
                            "(\n" +
                            "\t(tbl.DT_KD>TIMESTAMP('1900-01-01','00:00') and {fn timestampdiff(SQL_TSI_MINUTE,tbl.DT_ND,tbl.DT_KD)}>=60)\n" +
                ...
                { //Передержка
                    Map<String,Object> addInParams= new HashMap<String,Object>();
                    final String tblName1 = TablesTypes.WINDOWS_OVERTIME;
                    addInParams.put("EVENT_TYPE", tblName1);
                    String addStringFilter="";
                    sqlConsolidator(conn, outTuples, tblName1, addStringFilter, addInParams, "CNT");

                    sqlDorConsolidator(conn, outTuples, tblName1, addStringFilter, null, addInParams,dummyHM,false, "CNT");
                    sqlDorConsolidator(conn, outTuples, tblName1, addStringFilter, null, addInParams,dummyHM,true, "CNT");


                    addStringFilter="ID_TUCH<>0 ";
                    addInParams.put("EVENT_TYPE",addInParams.get("EVENT_TYPE")+"P");
                    sqlConsolidator(conn, outTuples, tblName, addStringFilter, addInParams, "CNT2");
                    sqlDorConsolidator(conn, outTuples, tblName, addStringFilter, null, addInParams,dummyHM,false, "CNT2");
                    sqlDorConsolidator(conn, outTuples, tblName, addStringFilter, null, addInParams,dummyHM,true, "CNT2");
                ...
                { //Звершено всего
                    Map<String,Object> addInParams= new HashMap<String,Object>();
                    addInParams.put("EVENT_TYPE",tblName+"#CL");
                    String addStringFilter="STATUS_FACT=2  and STATUS_PL<>1\n" +
                            "and tbl.KD>=TIMESTAMP('"+stringDate+"','00:00')  \n" +
                            "and \n" +
                            "(\n" +
                            "\t(tbl.DT_KD>TIMESTAMP('1900-01-01','00:00') and {fn timestampdiff(SQL_TSI_MINUTE,tbl.DT_ND,tbl.DT_KD)}>=60)\n" +
...
//    }
    ...
    private void sqlConsolidator(Connection conn, List<Map> outTuples, String tblName, String addFilter, Map<String, Object> addParams, String cntName) {
        ...
        try
        {
            ...
            query = "select pl.DOR_KOD,pl.POLG_ID,pl.NAME as PNAME,pl.NUM,trim ( char(pl.DOR_KOD)) || '##' || trim ( char(pl.POLG_ID)) as " + TablesTypes.PLACE_ID + " ,count( distinct trim ( char(tdp.DATATYPE_ID)) ||'##' || tdp.DATA_OBJ_ID || '##' || trim(char(po.POLG_ID)) ) as " + cntName + " \n" +
                    " from "+tblName+"_PLACES tdp,"+tblName+" tbl,POLG_OBJ po, TABLO_POLG_LIST pl\n" +
                    "where po.POLG_TYPE=100185 and po.OBJ_OSN_ID=tdp.OBJ_OSN_ID and pl.POLG_ID=po.POLG_ID and pl.DOR_KOD<>0 and pl.DOR_KOD=tbl.DOR_KOD and pl.POLG_TYPE=100185 \n" +
                    "and tbl.DATA_OBJ_ID=tdp.DATA_OBJ_ID "+addFilter+" \n" +
                    "group by pl.POLG_ID,pl.NAME,pl.DOR_KOD,pl.NUM order by DOR_KOD,pl.NUM";
