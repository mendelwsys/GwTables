...
import com.mwlib.tablo.db.ServerUpdaterUNEW;
 ...
 * Date: 25.09.14
 * Time: 12:37
 * Сервлет подкачики данных в таблицы клиентов (С имплементацией самых разных тестов)
 */
public class DataServletWK extends HttpServlet {


    private ServerUpdaterUNEW updater;
    ...
    public void init() throws ServletException
    {
        ...
        try
        {
            ...
            TablesTypes.TESTMODES testMode = TablesTypes.TESTMODES.valueOf(ServletUtils.setInitParamByParams(getServletConfig().getInitParameter(TablesTypes.TESTMODEVAL), TablesTypes.TESTMODES.SILENCE.getValue()));

            String dsName = ServletUtils.setName(getServletConfig().getInitParameter(TablesTypes.DS_NAME), DbUtil.DS_ORA_NAME);
            String dsCacheName = ServletUtils.setName(getServletConfig().getInitParameter(TablesTypes.DS_CACHE_NAME), DbUtil.DS_JAVA_CACHE_NAME);
            String testTable = ServletUtils.setName(getServletConfig().getInitParameter(TablesTypes.TESTTABLE),TablesTypes.VIOLATIONS);
            ...
            Directory.initDictionary(test,dsCacheName);
            ...
            updater = new ServerUpdaterUNEW(eventProvider, IDerbyCliProviderFactoryImpl.getCliManagerInstance(), new DerbyDefaultCacheFactory(),params);
