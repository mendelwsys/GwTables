...
import com.mwlib.tablo.test.derby.SQLDerbyCliProvider;
 ...
 */
public class CliProviderFactoryImpl2
        implements ICliProviderFactory, ICliManager
{

    private static CliProviderFactoryImpl2 cliProviderFactory=new CliProviderFactoryImpl2();
    ...
    protected CliProviderFactoryImpl2()
    {
    ...
    public ICliProvider[] getProvider(Map parameters) throws WrongParam
    {
        ...
        if (parameters.containsKey(TablesTypes.REQPARAM))
        {
            String reqParams=getParameter(parameters, TablesTypes.REQPARAM);
            String recParam = getParameter(parameters, "RECPARAM");
            if (reqParams!=null)
            {
                ...
                Type type = new TypeToken<Map<String, GWTSuccs<Object>>>(){}.getType();
                Map<String,GWTSuccs<Object>> pparam=gson.fromJson(recParam, type);

                for (String key : pparam.keySet())
                    parameters.put(key,pparam.get(key).val);
                ...
                CliProvider cliProvider;
                {
                    ...
                    if (cliProvider ==null)
                    {
                        String[] inTypes=params.getInTblTypes();
                        List<String> ll=new LinkedList<String>(Arrays.asList(inTypes));
                        ll.add(tType);

                        Pair<ICache, BaseTableDesc> cachePair = name2Cache.get(tType);
                        cliProvider = new SQLDerbyCliProvider(cachePair,cliId, tblId, ll.toArray(new String[ll.size()]));
                        rv.put(tblId,cliProvider);
                        for (String inType : inTypes)
                        {
                            Map<String,CliProvider> rv1 =tType2TblN2Providers.get(inType);
                            if (rv1==null)
                                tType2TblN2Providers.put(inType, rv1 = new ConcurrentHashMap<String, CliProvider>());
                            rv1.put(tblId,cliProvider);
                        }
                    }
                    return new ICliProvider[]{cliProvider};
                }
            }
            return new ICliProvider[]{};
        }
        else
