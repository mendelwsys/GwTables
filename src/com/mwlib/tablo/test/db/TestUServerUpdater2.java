 ...
 */
public class TestUServerUpdater2
{
    public static void main(String[] args) throws Exception
    {
        ...
        Directory.initDictionary(test);
        {
            ...
            IEventProvider[] eventProviders = new IEventProvider[]{new PlacesEventProvider(metaProvider),new EventProviderTImpl(metaProvider),new ObjectsEventProvider(metaProvider)};

            ServerUpdaterU2.ProvidersWrapper[] eventProviderWrappers = new ServerUpdaterU2.ProvidersWrapper[eventProviders.length];
            for (int i = 0, eventProvidersLength = eventProviders.length; i < eventProvidersLength; i++)
                eventProviderWrappers[i]=new ServerUpdaterU2.ProvidersWrapper(eventProviders[i]);
            eventProviderWrappers[eventProviders.length-1].setMaster(true);

            final List<DerbyCache> ll=new LinkedList<DerbyCache>();

            ServerUpdaterU2 updater01 = new ServerUpdaterU2(eventProviderWrappers, ICliProviderFactoryImpl.getCliManagerInstance(),new ICacheFactory() {
                @Override
                    ...
                    for (DerbyCache derbyCache : ll)
                    {
                        try
                        {
                            ...
                            for (int i = 0, metaLength = meta.length; i < metaLength; i++)
                            {
                                ...
                                if (columnHeadBean.getName().equals(PROV + "_0"))
                                {
                                    ix2 = i;
                                }
                                else
                                if (columnHeadBean.getName().equals(PROV + "_1"))
                                {
                                    ix = i;
                                }
                                if (ix>=0 && ix2>=0)
                            ...
                            if (ix>=0)
                            {
                                Object[][] res=derbyCache.getTuplesByParameters(new HashMap(), null);
                                for (Object[] re : res)
                                {
