 ...
 */
public class PlacesEventProvider implements IEventProvider
{
...
//          return " IS NOT NULL ";
    ...
    public PlacesEventProvider(IMetaProvider metaProvider, String dsName)
    {
        this.dsName=dsName;
        initIt(metaProvider);
    ...
    public PlacesEventProvider(IMetaProvider metaProvider)
    {
        initIt(metaProvider);
    }
    ...
    protected Pair<IMetaProvider, Map[]> getDataPlaces(String placesSQL, Map<String, ParamVal> mapParams, Map<String, Object> outParams, int actual) throws ClassNotFoundException, SQLException
    {
        ...
        try
        {
            ...
            while (rs.next())
            {
                ...
                Integer obj_osn_id=rs.getInt(TablesTypes.OBJ_OSN_ID);
                String key=idEvent+ TablesTypes.KEY_SEPARATOR +dataTypeId.toString()+ TablesTypes.KEY_SEPARATOR +obj_osn_id.toString();//ключ объекта должен быть сформирован так же.
                ...
                {
                    tuple.put(TablesTypes.PUTGL_ID,null);
                    tuple.put(TablesTypes.OBJ_OSN_ID,null);
                }
            ...
            System.out.println(this.getClass().getName()+" :count of TABLO_DATA_PLACES records = " + nRec +" tuples:" + key2tuple.size()+" tuples time:"+((System.currentTimeMillis()-lg)/10)*1.0/100);
    ...
    {
        return "select DATA_OBJ_ID,"+ TablesTypes.CORTIME+",OBJ_OSN_ID,PUTGL_ID,"+TablesTypes.DOR_CODE+","+TablesTypes.DATATYPE_ID+" " +
                        "from ICG0.TABLO_DATA_PLACES WHERE COR_TIP " + corTip + dor_kod+
