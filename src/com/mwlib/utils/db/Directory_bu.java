...
public class Directory_bu
{
    ...
    public synchronized static void initDictionary(boolean test) throws Exception
    {

        test=true;//TODO На сервере Нет доступа к ЦНСИ!!!!

        if (isinit)
            return;

        Directory_bu.test=test;
        if (test)
        {
            //ObjectInputStream ois = new ObjectInputStream(new FileInputStream(fileTestName));
            InputStream inputStream = Directory_bu.class.getResourceAsStream(fileTestName);
            if (inputStream==null)
...
//                conn = DbUtil.getConnection2(DbUtil.DS_DB2_NAME);
                ...
                {
                    System.out.println(" error got connection ");
                    return;
                }
